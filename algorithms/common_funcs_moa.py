...
def extract_last_actions_from_episodes(episodes, batch_type=False, own_actions=None):
    ...
    """
    assert episodes is not None, "Empty episode buffer"
...
class MOAResetConfigMixin(object):
    ...
    def reset_policies(policies, new_config, session):
        ...
        for policy in policies:
            policy.moa_loss_weight.load(custom_model_config["moa_loss_weight"], session=session)
            policy.compute_influence_reward_weight = lambda: custom_model_config[
